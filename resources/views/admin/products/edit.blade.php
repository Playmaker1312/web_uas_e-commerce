...
<body class="min-h-screen flex bg-gradient-to-br from-[#232946] to-[#1a223f]">
    ...
    <main class="flex-1 flex flex-col min-h-screen px-2 md:px-8 py-4">
        ...
        <div class="flex flex-col items-center justify-center flex-1">
            <form method="POST" action="{{ route('products.update', $product) }}" enctype="multipart/form-data" class="form-glass relative">
                <h1><i class="bi bi-box-seam"></i> Edit Produk</h1>
                @csrf
                @method('PUT')
                @if ($errors->any())
                ...
                <div class="mb-8 relative">
                    <input type="text" name="name" id="name" autofocus value="{{ old('name', $product->name) }}" class="input-glass peer" required placeholder=" " />
                    <label for="name" class="label-glass">Nama Produk</label>
                    <span class="absolute right-3 top-2 text-xs text-blue-300" title="Nama produk yang akan ditampilkan di toko."><i class="bi bi-info-circle"></i></span>
                    @error('name')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                </div>
                <div class="mb-8 relative">
                    <select name="category_id" id="category_id" class="input-glass peer" required>
                        <option value="">Pilih Kategori</option>
                        @foreach ($categories as $cat)
                            <option value="{{ $cat->id }}" @if($product->category_id == $cat->id) selected @endif>{{ $cat->name }}</option>
                        @endforeach
                    </select>
                    <label for="category_id" class="label-glass">Kategori</label>
                    <span class="absolute right-3 top-2 text-xs text-blue-300" title="Pilih kategori produk."><i class="bi bi-info-circle"></i></span>
                </div>
                <div class="mb-8 relative">
                    <input type="number" name="price" id="price" value="{{ $product->price }}" class="input-glass peer" required placeholder=" " />
                    <label for="price" class="label-glass">Harga</label>
                    <span class="absolute right-3 top-2 text-xs text-blue-300" title="Harga dalam rupiah."><i class="bi bi-cash"></i></span>
                </div>
                <div class="mb-8 relative">
                    <input type="number" name="stock" id="stock" value="{{ $product->stock }}" class="input-glass peer" required placeholder=" " />
                    <label for="stock" class="label-glass">Stok</label>
                    <span class="absolute right-3 top-2 text-xs text-blue-300" title="Jumlah stok produk."><i class="bi bi-box"></i></span>
                </div>
                <div class="mb-8 relative">
                    <input type="file" name="image" id="image" accept="image/*" class="input-glass peer" onchange="previewImage(event)" placeholder=" " />
                    <label for="image" class="label-glass">Gambar Produk</label>
                    <span class="absolute right-3 top-2 text-xs text-blue-300" title="Upload gambar produk."><i class="bi bi-image"></i></span>
                    @error('image')<span class="text-red-500 text-xs">{{ $message }}</span>@enderror
                    @if($product->image)
                        <img src="{{ asset('storage/' . $product->image) }}" alt="{{ $product->name }}" class="img-preview-glass" id="img-preview" />
                    @else
                        <img id="img-preview" class="img-preview-glass hidden" />
                    @endif
                </div>
                <div class="flex justify-between gap-2 mt-8">
                    <a href="{{ route('products.index') }}" class="btn-glass secondary flex items-center gap-1"><i class="bi bi-arrow-left"></i> Kembali</a>
